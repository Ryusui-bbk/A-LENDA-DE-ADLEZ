...
export class Enemy extends Phaser.Physics.Arcade.Sprite {
  ...
  _chase(player) {
    ...
    this.play(`orc-walk-${this.direction}`, true);
  }
  ...
  _idle(delta) {
    ...
    if (this.wanderDirX === 0 && this.wanderDirY === 0) {
      this.play(`orc-idle-${this.direction}`, true);
    } else {
  ...
  takeDamage(amount, source) {
    ...
    if (source) {
      ...
      this.setVelocity(
        Math.cos(angle) * CONFIG.KNOCKBACK_FORCE,
        Math.sin(angle) * CONFIG.KNOCKBACK_FORCE,
      );
  ...
  die() {
    ...
    this.scene.tweens.add({
      ...
      ease: 'Power2',
      onComplete: () => {
        // Spawn emerald at death position
        if (this.scene && this.scene.spawnEmerald) {
          this.scene.spawnEmerald(this.x, this.y);
        }
